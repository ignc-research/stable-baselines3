...
# found_rl_agent = True
# try:
#     from arena_marl.marl_agent.envs.pettingzoo_env import FlatlandPettingZooEnv
# except ModuleNotFoundError:
#     warnings.warn("Couldn't import 'rl_agent.envs.pettingzoo_env'! Thus, excluding method 'evaluate_marl_policy().'")
...
def evaluate_marl_policy(
    ...
    episode_rewards, episode_lengths = [], []
    not_reseted = True
    agents = env.possible_agents if not is_vec_env else get_vecagent_list(env)
    ...
    while len(episode_rewards) < n_eval_episodes:
        ...
        episode_length = 0
        success_count = 0
        tmp_episode_length = []  # temporarily save steps when a robot is done in order to calc mean
        ...
        while len(done_agents) < num_robots:
            ...
            actions = {
                agent: model.predict(obs[agent], state=state, deterministic=deterministic)[0] for agent in agents
            }
            if is_vec_env:
            ...
            if is_vec_env:
                (
                    ...
                    dones,
                    infos,
                ) = vecenv_multiple_listsarrays2dicts(agents, [obs, rewards, dones, infos])

            curr_dones, curr_done_agents = extract_dones_2(infos, done_agents)
...
def vecenv_action_dict2array(data: dict, done_agents: list) -> np.ndarray:
    ...
    return np.stack(
        [data_array if agent not in done_agents else np.array([0.0, 0.0]) for agent, data_array in data.items()]
    )


def vecenv_multiple_listsarrays2dicts(agent_list: list, data: list) -> tuple:
    return tuple(vecenv_listarray2dict(agent_list, data_type) for data_type in data)
...
def extract_dones_2(infos: Dict[str, Any], done_agents: List[str]) -> Tuple[int, List[str]]:
    done_agents = [agent for agent, info in infos.items() if "done_reason" in info and agent not in done_agents]
    return len(done_agents), done_agents


def extract_successes(infos: Dict[str, Any]) -> int:
    return sum(reason["done_reason"] == 2 for reason in infos.values() if "done_reason" in reason)


def sum_rewards(rewards: Dict[str, float], done_agents: List[str]) -> float:
    return sum(reward for agent, reward in rewards.items() if agent not in done_agents)
